...
public:
	...
	void Handler(uint8_t DataType, uint8_t Note, uint8_t Velo, uint8_t Channel);
	int ListLength();
	void ListSendNoteOn();
	void Arp();
...
extern "C" {
	...
	int init_midi(void);
	void midi_task(void);
	extern void MidiHandler(uint8_t DataType, uint8_t Note, uint8_t Velo, uint8_t Channel);
	e_mode getArpMode();
	void setArpMode(e_mode mode);
	uint8_t getMidiRX();
	uint8_t getMidiTX();
